...
function LegislationExamContent() {
  ...
  const examTitle = lang === 'ar'
    ? 'الاستعداد للامتحان - تشريعات البناء'
    : 'הכנה למבחן - תחיקת הבנייה'

  async function startSession() {
    setLoading(true)
    setErrorMsg(null)
    try {
      const res = await fetch('/api/legislation/generate-question', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ questionType: qType, sessionId: null, isFirst: true, language: lang }),
      })
  ...
  async function nextQuestion() {
    ...
    try {
      const res = await fetch('/api/legislation/generate-question', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ questionType: qType, sessionId, language: lang }),
      })
      const data = await res.json()
      if (data.question) {
        setCurrentQ(data.question)
  ...
  async function submitAnswer() {
    ...
    try {
      ...
      const res = await fetch('/api/legislation/grade-answer', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ question: currentQ, studentAnswer, sessionId, language: lang }),
      })
      const data = await res.json()
      setResult(data)
  ...
  if (phase === 'setup') {
    return (
      <div>
        <h1 className="text-2xl font-bold mb-6" style={{ color: '#2d6a4f' }}>{examTitle}</h1>
        <div className="card p-6 max-w-lg mx-auto">
            ...
            <div className="flex gap-2">
              {[['mixed', t.exam.mixed], ['multiple', t.exam.multiple], ['open', t.exam.open]].map(([v, l]) => (
                <button key={v} onClick={() => setQType(v as typeof qType)}
                  ...
                  style={qType === v ? { backgroundColor: '#2d6a4f', borderColor: '#2d6a4f' } : {}}>
                  {l}
                </button>
              ))}
            </div>
          ...
          {errorMsg && <p className="text-sm mt-3 text-center" style={{ color: 'var(--error)' }}>{errorMsg}</p>}
        </div>
      </div>
    )
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-bold" style={{ color: '#2d6a4f' }}>{examTitle}</h1>
        ...
        <div className="card p-6">
            ...
            <div className="space-y-2 mb-5">
              {currentQ.choices.map((choice, i) => {
                ...
                if (phase === 'result') {
                  ...
                  style = { border: '2px solid #2d6a4f', backgroundColor: '#f0fff4' }
                }
                return (
          ...
          {phase === 'question' && (
            <button onClick={submitAnswer}
              disabled={submitting || (currentQ.type === 'multiple' ? selectedIdx === null : !answer.trim())}
          ...
          {phase === 'result' && result && (
            <div className="mt-4 space-y-3">
              <div className={`p-4 rounded-xl border-2 ${result.score === 10 ? 'score-10 border-green-200' : result.score === 5 ? 'score-5 border-yellow-200' : 'score-0 border-red-200'}`}>
                <div className="flex items-center gap-3 mb-2">
                  <span className="score-badge" style={
                    result.score === 10 ? { backgroundColor: '#276749', color: 'white' }
                    : result.score === 5 ? { backgroundColor: '#975a16', color: 'white' }
              ...
              <button onClick={nextQuestion} className="btn-primary" style={{ backgroundColor: '#2d6a4f' }}>
                {t.exam.nextQuestion}
              </button>
            </div>
          )}
